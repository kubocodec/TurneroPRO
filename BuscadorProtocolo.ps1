...
try {
    ...
    $port.RtsEnable = $true
    $port.Open()
    Write-Host "Puerto $portName abierto. Iniciando escaneo de comandos..."
    ...
    foreach ($cmd in $commands) {
        ...
        Write-Host "Observa si la luz ACTIVE se enciende..."
        
        $port.Write($cmd, 0, $cmd.Length)
        
        # Esperar 4 segundos por comando
        for ($i=0; $i -lt 40; $i++) {
            if ($port.BytesToRead -gt 0) {
                ...
                Write-Host "    [!] RESPUESTA RECIBIDA DEL DISPOSITIVO: $respHex"
            }
